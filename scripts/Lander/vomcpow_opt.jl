...
start_mean = [10.0, 3.0, 4.0, 2.0, 10.0, 0.8]
start_cov = diagm([10.0^2, 5.0^2, 5.0^2, 5.0^2, 5.0^2, 0.1^2])
...
function gen_sims(x::Vector{Float64}, n, k, seed)
    ...
    for i in 1:n
        ...
        filter = EKFUpdater(pomdp, pomdp.Q.^2, pomdp.R.^2)
...
try
  for i in 1:max_iters
      ...
      if save_data
        fname_csv = joinpath(@__DIR__, "../data", "Lander_vomcpow_opt"*"_"*datestring*".csv")
